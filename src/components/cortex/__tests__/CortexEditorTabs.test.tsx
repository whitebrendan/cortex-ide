...
describe("CortexEditorTabs", () => {
  ...
  describe("Active Tab", () => {
    it("should highlight active tab with different background", () => {
      ...
      expect(activeTab?.style.background).toContain("--cortex-bg-elevated");
      expect(inactiveTab?.style.background).toBe("transparent");
    });

    it("should style active tab with bg and border-radius (rounded top only per Figma)", () => {
      ...
      // Active tab uses CSS variable for bg with fallback and 8px 8px 0 0 border-radius per Figma
      expect(activeTab?.style.background).toContain("--cortex-bg-elevated");
